...
extendt-Fin₂ [] B' C' n v with down _ n
... | nothing = inject₁ v
... | just n' = v
extendt-Fin₂ (a ∷ A') B' C' n v with down _ n
... | nothing = v
... | just n' = extendt-Fin₂ A' B' C' n' v

extendt-Fin : ∀{n m} → (S : Vec ℕ n m) → (T : Vec ℕ zero n) → (gid : Fin n) → (gid₂ : Fin m) → Fin (lookup gid₂ (S ++ T)) → Fin (lookup gid₂ (S ++ T [ gid ]≔ suc (lookup gid T)))
extendt-Fin S [] () gid₂ x
extendt-Fin S (x ∷ T) gid₁ gid₂ x₁ with down _ gid₁ 
... | nothing = extendt-Fin₂ S [] T gid₂ x₁
... | just v rewrite append-concat x T S | append-concat x (T [ v ]≔ suc (lookup v T)) S = extendt-Fin (S ∷ʳ x) T v gid₂ x₁
...
extendt-val S T idx (A {n} x) = A (extendt-Fin S T idx n x)
...
extendt-Cfg₂ _ _ ._ [] = []
extendt-Cfg₂ A' B' (c ∷ C') (x ∷ cfg) rewrite append-concat c C' B'
...
... | just gid' rewrite append-concat t (T [ gid' ]≔ suc (lookup gid' T)) S | append-concat t T S = x ∷ extendt-Cfg' (S ∷ʳ t) T gid' task cfg
... | nothing with x
...
extendt-CfgE' A' B' [] ()
extendt-CfgE' A' B' (c ∷ C) (head-E x) rewrite append-concat c C B' = head-E (extendt-Cfg₂ A' (B' ∷ʳ c) C x)
extendt-CfgE' A' B' (c ∷ C) (tail-E x cfgE) rewrite append-concat c C B' = tail-E (extendt-group₂ A' (B' ∷ʳ c) C x) (extendt-CfgE' A' (B' ∷ʳ c) C cfgE)
...
... | just gid' rewrite append-concat t T S | append-concat t (T [ gid' ]≔ suc (lookup gid' T)) S = extendt-group T (S ∷ʳ t) gid' x ∷ extendt-Cfg (S ∷ʳ t) T gid' cfg
...
... | just gid' rewrite append-concat t T S | append-concat t (T [ gid' ]≔ suc (lookup gid' T)) S = head-E (extendt-Cfg (S ∷ʳ t) T gid' x)
... | nothing = head-E (extendt-Cfg₂ S [] T x)
extendt-CfgE {n = suc n} S (t ∷ T) gid (tail-E x cfgE) with down n gid
... | just gid' rewrite append-concat t T S | append-concat t (T [ gid' ]≔ suc (lookup gid' T)) S = tail-E (extendt-group T (S ∷ʳ t) gid' x) (extendt-CfgE (S ∷ʳ t) T gid' cfgE)
